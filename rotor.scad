...
module subtractor() {
  rotate([0, 0, -22.5]) {
    cube([rotor_edge/2, rotor_diameter + 20, rotor_height]);
  }
}

module rotor() {
  difference() {
    translate([-rotor_edge/2, 0, 0]) { cube([rotor_edge, rotor_diameter, rotor_height]); }
    subtractor();
    ...
    translate([30, rotor_diameter - 10, 0]) { cylinder(h=rotor_height, r=hole_radius); }
    translate([-30, rotor_diameter - 10, 0]) { cylinder(h=rotor_height, r=hole_radius); }
